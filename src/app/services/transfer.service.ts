...
import { MultiTransfer, Transfer } from 'src/Models/Transfer';
...
export class TransferService {
  ...
  ajouter(transfer: Transfer) {
    ...
    if(!verify.error) {
      ...
      if(!verify.error) {
        ...
        this.multiTransfer.total_expense_amount = Math.round(this.multiTransfer.total_amount * 0.05);
        return true;
      }
  ...
  supprimer(transfer: Transfer) {
    this.multiTransfer.transfers = this.multiTransfer.transfers.filter((element: Transfer) => { return element != transfer });
    this.multiTransfer.total_amount -= transfer.transfer_amount;
    this.multiTransfer.total_expense_amount = Math.round(this.multiTransfer.total_amount * 0.05);
  }
  ...
  NameAlreadyExist(transfer : Transfer){
    var someArr = this.multiTransfer.transfers.filter( (val:any) => {
      if(val.receiver_fname == transfer.receiver_fname && val.receiver_lname == transfer.receiver_lname) {
        return val;
      }
      return null;
    } );
  ...
  PhoneAlreadyUsed(transfer : Transfer){
    ...
    return { error: someArr.length != 0, message: "Numéro de téléphone déjà utilisé!" };
  }
}
